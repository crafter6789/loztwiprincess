...
public class TeleporterTwilightPrincess
{
    private final WorldServer worldServerInstance;
    /** A private Random() function in Teleporter */
    private final Random random;
    private final Long2ObjectMap<TeleporterTwilightPrincess.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);

    public TeleporterTwilightPrincess(WorldServer worldIn)
    {
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw)
    {
        ...
        {
            TeleporterTwilightPrincess.PortalPosition teleporter$portalposition = (TeleporterTwilightPrincess.PortalPosition)this.destinationCoordinateCache.get(l);
            d0 = 0.0D;
        ...
        if (d0 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.put(l, new TeleporterTwilightPrincess.PortalPosition(blockpos, this.worldServerInstance.getTotalWorldTime()));
            }
    ...
    public void removeStalePortalLocations(long worldTime)
    {
        if (worldTime % 100L == 0L)
        {
            ...
            ObjectIterator<TeleporterTwilightPrincess.PortalPosition> objectiterator = this.destinationCoordinateCache.values().iterator();

            while (objectiterator.hasNext())
            {
                TeleporterTwilightPrincess.PortalPosition teleporter$portalposition = (TeleporterTwilightPrincess.PortalPosition)objectiterator.next();
